...
class Edrone():
    ...
    def __init__(self):
        ...
        self.Kp = [118.2, 118.2, 1297.2]
        self.Ki = [0, 0, 0]
        self.Kd = [38.64, 38.64, 0]
        # -----------------------Add other required variables for pid here ----------------------------------------------
        self.throttle_cmd = 0
        self.prop_throttle = 0
        self.error = [0.0, 0.0, 0.0]
        self.prev_error = [0.0, 0.0, 0.0]
        self.error_difference = [0.0, 0.0, 0.0]
        self.max_prop_speed = 1023
        self.min_prop_speed = 0
        self.error_sum = [0, 0, 0]
        self.p_output = [0.0, 0.0, 0.0]
        self.i_output = [0.0, 0.0, 0.0]
        self.d_output = [0.0, 0.0, 0.0]
        self.pid_output = [0.0, 0.0, 0.0]
    ...
    #def yaw_set_pid(self, yaw):
        #self.Kp[2] = yaw.Kp * 0.6  # This is just for an example. You can change the ratio/fraction value accordingly
    ...
    def pid(self):
        ...
        #self.prop_throttle = self.throttle_cmd * 1.024 - 1024
        self.prop_throttle = self.throttle_cmd
        ...
        self.pid_output[0] = (self.error[0] * self.Kp[0]) + (self.error_difference[0] * self.Kd[0]/self.sample_time) + (self.error_sum[0] * self.Ki[0])
        self.pid_output[1] = (self.error[1] * self.Kp[1]) + (self.error_difference[1] * self.Kd[1]/self.sample_time) + (self.error_sum[1] * self.Ki[1])
        self.pid_output[2] = (self.error[2] * self.Kp[2]) + (self.error_difference[2] * self.Kd[2]/self.sample_time) + (self.error_sum[2] * self.Ki[2])

        self.pwm_cmd.prop1 = self.prop_throttle - self.pid_output[0] + self.pid_output[1] - self.pid_output[2]
        self.pwm_cmd.prop2 = self.prop_throttle - self.pid_output[0] - self.pid_output[1] + self.pid_output[2]
        self.pwm_cmd.prop3 = self.prop_throttle + self.pid_output[0] - self.pid_output[1] - self.pid_output[2]
        self.pwm_cmd.prop4 = self.prop_throttle + self.pid_output[0] + self.pid_output[1] + self.pid_output[2]

        if self.pwm_cmd.prop1 > self.max_prop_speed:
            self.pwm_cmd.prop1 = self.max_prop_speed
        elif self.pwm_cmd.prop1 < self.min_prop_speed:
            self.pwm_cmd.prop1 = self.min_prop_speed
        
        if self.pwm_cmd.prop2 > self.max_prop_speed:
            self.pwm_cmd.prop2 = self.max_prop_speed
        elif self.pwm_cmd.prop2 < self.min_prop_speed:
            self.pwm_cmd.prop2 = self.min_prop_speed

        if self.pwm_cmd.prop3 > self.max_prop_speed:
            self.pwm_cmd.prop3 = self.max_prop_speed
        elif self.pwm_cmd.prop3 < self.min_prop_speed:
            self.pwm_cmd.prop3 = self.min_prop_speed

        if self.pwm_cmd.prop4 > self.max_prop_speed:
            self.pwm_cmd.prop4 = self.max_prop_speed
        elif self.pwm_cmd.prop4 < self.min_prop_speed:
            self.pwm_cmd.prop4 = self.min_prop_speed
